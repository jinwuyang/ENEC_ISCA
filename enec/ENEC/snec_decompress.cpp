...
public:
    ...
    )
    {
        ...
        pipe->InitBuffer(mbl_inQueue, BUFFER_NUM, tileLength * tileNum / 8); 

        pipe->InitBuffer(compPrefix, BLOCK_NUM * sizeof(uint32_t)); 
        ...
        int FinalOtherSize = 32 +
                            datablockSize / 2 * datablockNum +
                            (datablockSize / sizeof(uint16_t)) * 3 / 8 * datablockNum +
                            tileNum / 8 * datablockNum + 
                            BLOCK_NUM * 4; 

        this->threadcompedNum = ((blockId == BLOCK_NUM - 1 ? totalCompressedBytes - FinalOtherSize : compPrefixLocal(blockId + 1)) - compPrefixLocal(blockId)) * 8 / 3;
    }
    ...
    __aicore__ inline void Process()
    {
        ...
        pipe->InitBuffer(offset0, tileNum / 8);         
        ...
        pipe->InitBuffer(temp1, 32 + computeNum * sizeof(float)); 
        ...
        AIV_WITH_BARRIER(DataCopy, mergeLocal, e_input1, computeNum * 3 / 16);
...
private:
    ...
    __aicore__ inline void Compute(int32_t i,
                                   ...
                                   LocalTensor<T> &offset0Local, 
                                   LocalTensor<T> &offset1Local,
                                   LocalTensor<T> &mask1Local  
                                )
    {
        ...
        AIV_WITH_BARRIER(Add, tempLocal0[computeNum].template ReinterpretCast<half>()[16], tempLocal0.template ReinterpretCast<half>(), tempLocal0.template ReinterpretCast<half>()[16], tileNum - 16);
        AIV_WITH_BARRIER(Add, tempLocal0[computeNum].template ReinterpretCast<half>()[32], tempLocal0[computeNum].template ReinterpretCast<half>(), tempLocal0[computeNum].template ReinterpretCast<half>()[32], tileNum - 32);
        AIV_WITH_BARRIER(Add, tempLocal0[computeNum].template ReinterpretCast<half>()[64], tempLocal0[computeNum].template ReinterpretCast<half>(), tempLocal0[computeNum].template ReinterpretCast<half>()[64], tileNum - 64);
        AIV_WITH_BARRIER(Add, tempLocal0[computeNum].template ReinterpretCast<half>()[128], tempLocal0[computeNum].template ReinterpretCast<half>(), tempLocal0[computeNum].template ReinterpretCast<half>()[128], tileNum - 128);
        AIV_WITH_BARRIER(Add, tempLocal0[computeNum].template ReinterpretCast<half>()[256], tempLocal0[computeNum].template ReinterpretCast<half>(), tempLocal0[computeNum].template ReinterpretCast<half>()[256], tileNum - 256);
        AIV_WITH_BARRIER(Add, tempLocal0[computeNum].template ReinterpretCast<half>()[512], tempLocal0[computeNum].template ReinterpretCast<half>(), tempLocal0[computeNum].template ReinterpretCast<half>()[512], tileNum - 512);
        ...
        if(accouterNum + outerNum >= computeNum)
        {
            ...
            AIV_WITH_BARRIER(DataCopy, mergeLocal, e_input1[accCompressed  * 3 / 8 / sizeof(T)], computeNum0 * 3 / 8 / sizeof(T));
        ...
        AIV_WITH_BARRIER(Adds, tempLocal0[computeNum].template ReinterpretCast<int16_t>(), tempLocal0[computeNum].template ReinterpretCast<int16_t>(), (int16_t)123, computeNum);
...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        AIV_WITH_BARRIER(ShiftLeft, mergeLocal, mergeLocal, (uint16_t)14, computeNum / 2);
        AIV_WITH_BARRIER(ShiftRight, mergeLocal, mergeLocal, (uint16_t)14, computeNum / 2);
