...
public:
    __aicore__ inline void Process()
    {
        ...
        AIV_WITH_BARRIER(Add, tempLocal[8].template ReinterpretCast<int32_t>(), tempLocal.template ReinterpretCast<int32_t>(), tempLocal[8].template ReinterpretCast<int32_t>(), 8 * 64 - 8);
        AIV_WITH_BARRIER(Add, tempLocal[8 * 2].template ReinterpretCast<int32_t>(), tempLocal.template ReinterpretCast<int32_t>(), tempLocal[8 * 2].template ReinterpretCast<int32_t>(), 8 * 64 - 8 * 2);
        AIV_WITH_BARRIER(Add, tempLocal[8 * 4].template ReinterpretCast<int32_t>(), tempLocal.template ReinterpretCast<int32_t>(), tempLocal[8 * 4].template ReinterpretCast<int32_t>(), 8 * 64 - 8 * 4);
        AIV_WITH_BARRIER(Add, tempLocal[8 * 8].template ReinterpretCast<int32_t>(), tempLocal.template ReinterpretCast<int32_t>(), tempLocal[8 * 8].template ReinterpretCast<int32_t>(), 8 * 64 - 8 * 8);
        AIV_WITH_BARRIER(Add, tempLocal[8 * 16].template ReinterpretCast<int32_t>(), tempLocal.template ReinterpretCast<int32_t>(), tempLocal[8 * 16].template ReinterpretCast<int32_t>(), 8 * 64 - 8 * 16);
        AIV_WITH_BARRIER(Add, tempLocal[8 * 32].template ReinterpretCast<int32_t>(), tempLocal.template ReinterpretCast<int32_t>(), tempLocal[8 * 32].template ReinterpretCast<int32_t>(), 8 * 64 - 8 * 32);
        ...
        uint32_t copystart0 = blockId * buffersize;
        ...
        if(blockId == 0){
            ...
            tempLocal[64](0) = datablocksize;                    
            tempLocal[64](1) = datablocknum;                      
            tempLocal[64](2) = threadblocknum | (complevel << 16);
            tempLocal[64](3) = totaluncompressedbytes;          
            tempLocal[64](4) = totalcompressedsize;          
            tempLocal[64](5) = tilelength | (datatype << 16);   
            tempLocal[64](6) = mbllength | (options << 16);      
            tempLocal[64](7) = histogrambytes;                 
